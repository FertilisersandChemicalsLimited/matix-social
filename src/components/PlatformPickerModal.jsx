          ...
          {PLATFORM_LIST.map((pid, i) => {
            ...
            return (
              <button
                key={pid}
                onClick={() => toggle(pid)}
                style={{ animationDelay: `${80 + i * 60}ms`, '--p': meta.color }}
                className={[
                  'relative rounded-2xl border-2 px-4 py-6 sm:py-8 transition-all duration-200 ease-snap animate-fade-up',
                  'flex flex-col items-center justify-center gap-3 group bg-white',
                  active
                    ? 'border-brand-500 shadow-glow scale-[1.02] bg-brand-50/60'
                    : 'border-cream-300/60 hover:border-brand-200 hover:-translate-y-0.5 hover:shadow-soft'
                ].join(' ')}
              >
                <span
                  className={[
                    'absolute top-3 right-3 h-5 w-5 rounded-full flex items-center justify-center transition-all',
                    active ? 'bg-brand-gradient text-white shadow-soft' : 'bg-cream-200 text-cream-200 group-hover:bg-cream-300'
                  ].join(' ')}
                >
                ...
                <span
                  className={[
                    ...
                    active ? 'shadow-soft' : 'opacity-80 group-hover:opacity-100'
                  ].join(' ')}
                  style={{
                    background: active ? `${meta.color}18` : '#fbf0e6',
                    color: meta.color
                  }}
                >
                  {ICONS[pid]}
                </span>
                <span className={`text-sm font-semibold ${active ? 'text-brand-700' : 'text-ink-900'}`}>
                  {meta.label}
                </span>
              </button>
            );
